(function (root, factory) {
  ...
  VMasker.toFloatNumber = function (value) {
    const separator = ",";
    const delimiter = ".";
    var number = value.toString().replaceAll(separator, "");
    const moneyArray = number.split(delimiter);
    var decima = 0;
    var money = "";
    var lastCharactor = number.substr(number.length - 1);

    if (moneyArray.length === 1) {
      money = money = moneyArray[0];
    ...
    if (masked.length) {
      var firstCharactor = masked.substr(0, 1);
      if (firstCharactor === separator) {
        masked = masked.replace(separator, "");
      }
      if (decima.length) {
        masked += delimiter + decima.substr(0, 1);
      }
      if (lastCharactor === delimiter) {
        masked += lastCharactor;
      }
      return masked;
    } else {
      return "0";
